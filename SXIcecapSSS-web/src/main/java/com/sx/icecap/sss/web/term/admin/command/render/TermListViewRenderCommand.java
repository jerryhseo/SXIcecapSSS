...
//		System.out.println("TermListViewRenderCommand.render()");
	...
	private boolean _trySearchThroughCategoryTree(
		...
		long assetCategoryId = ParamUtil.getLong(renderRequest, IcecapSSSWebKeys.CATEGORY_ID);
		String assetTagName = ParamUtil.getString(renderRequest, IcecapSSSWebKeys.TAG);
		
		if ((assetCategoryId != 0) || Validator.isNotNull(assetTagName)) {
			AssetEntryQuery assetEntryQuery = new AssetEntryQuery(Term.class.getName(), searchContainer);

			assetEntryQuery.setExcludeZeroViewCount(false);
			assetEntryQuery.setOrderByCol1(ParamUtil.getString(renderRequest, IcecapSSSWebKeys.ORDER_BY_COL));
			assetEntryQuery.setOrderByType1(ParamUtil.getString(renderRequest, IcecapSSSWebKeys.ORDER_BY_TYPE));
...
//			assetEntries.forEach(assetEntry -> entriesResults.add(_termLocalService.getTerm(assetEntry.getClassPK())));
	...
	private boolean _trySearchThroughService (
		...
		String Keywords = ParamUtil.getString( renderRequest, IcecapSSSWebKeys.KEYWORDS, StringPool.BLANK);
		if( !Keywords.isEmpty() ) {
			return false;
		}
		
		ThemeDisplay themeDisplay = (ThemeDisplay)renderRequest.getAttribute(WebKeys.THEME_DISPLAY);
		String navigation = ParamUtil.getString( renderRequest, IcecapSSSWebKeys.NAVIGATION, IcecapSSSConstants.NAVIGATION_ALL);
		...
		if (navigation.equals(IcecapSSSConstants.NAVIGATION_MINE)) {
		...
		else if( navigation.equals(IcecapSSSConstants.NAVIGATION_GROUP)){
			searchContainer.setTotal(
	...
	private boolean _trySearchWithIndexer(
		...
		ThemeDisplay themeDisplay = (ThemeDisplay)renderRequest.getAttribute(WebKeys.THEME_DISPLAY);
		String navigation = ParamUtil.getString( renderRequest, IcecapSSSWebKeys.NAVIGATION, IcecapSSSConstants.NAVIGATION_ALL);
		String orderByCol = ParamUtil.getString( renderRequest, IcecapSSSWebKeys.ORDER_BY_COL, IcecapSSSTermAttributes.TERM_NAME);
		String orderByType = ParamUtil.getString( renderRequest, IcecapSSSWebKeys.ORDER_BY_TYPE, IcecapSSSConstants.ASC);
		int status = ParamUtil.getInteger( renderRequest, IcecapSSSTermAttributes.STATUS, WorkflowConstants.STATUS_APPROVED);
	...
	private void _setSearchOptions(
		...
		searchContainer.setOrderByCol(
				ParamUtil.getString(
						renderRequest, 
						IcecapSSSWebKeys.ORDER_BY_COL, 
						IcecapSSSTermAttributes.TERM_NAME));
		searchContainer.setOrderByType(
				ParamUtil.getString(
						renderRequest, 
						IcecapSSSWebKeys.ORDER_BY_TYPE, 
						IcecapSSSConstants.ASC));
